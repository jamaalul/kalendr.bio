...
@section('content')
    <div class="flex lg:flex-row flex-col gap-8 w-full md:max-w-[80%]">
        ...
        <div class="flex flex-col gap-6 w-full lg:w-2/3">
            ...
            <div class="bg-white shadow-sm p-4 border rounded-xl w-full text-black" x-data="calendar()"
                ...
                <div class="gap-1 grid grid-cols-7">
                    ...
                    <template x-for="blank in blankdays">
                        <div class="p-2 border border-transparent h-16 md:h-24 text-center"></div>
                    </template>

                    {{-- Days of current month --}}
                    <template x-for="(date, dateIndex) in no_of_days" :key="dateIndex">
                        <div
                            class="group relative bg-white hover:bg-gray-50 p-1 md:p-2 border border-gray-100 rounded-lg h-20 md:h-28 transition-colors">
                            {{-- Date Number --}}
                            <span x-text="date"
                            ...
                            {{-- Events Dots/List --}}
                            <div class="flex flex-col gap-1 mt-1 max-h-[calc(100%-1.5rem)] overflow-y-auto no-scrollbar">
                                <template x-for="event in getEvents(date)">
                                    <div class="px-1.5 py-0.5 rounded text-[10px] md:text-xs truncate cursor-pointer"
                                        :class="getEventColor(event.status)" @click.stop="showEventDetails(event)">
                                        <span x-text="formatTime(event.starts_at)" class="opacity-75 font-semibold"></span>
            ...
            <div x-show="selectedEvent"
                class="z-50 fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 backdrop-blur-sm"
                x-transition.opacity style="display: none;">

                <div class="bg-white shadow-xl p-4 rounded-xl w-full max-w-md" @click.away="selectedEvent = null">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="font-medium text-sm">
                                    ...
                                    <span
                                        x-text="formatTime(selectedEvent?.starts_at) + ' - ' + formatTime(selectedEvent?.ends_at)"></span>
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="mt-6 pt-4 border-t text-center">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium text-xs"
                            :class="getEventColor(selectedEvent?.status)">
            ...
            <div class="flex flex-col gap-4">
                <div class="flex justify-between items-center">
                    ...
                    @if ($unacceptedBookings->count() > 0)
                        <span
                            class="bg-red-500 px-2 py-0.5 rounded-full font-bold text-white text-xs">{{ $unacceptedBookings->count() }}</span>
                    @endif
                </div>
                ...
                <div class="flex flex-col gap-3">
                    @forelse($unacceptedBookings as $booking)
                        <div
                            ...
                            <div class="gap-2 grid grid-cols-2 mt-1">
                                <form action="{{ route('booking.accept') }}" method="POST" class="w-full">
                                    @csrf
                                    <input type="hidden" name="id" value="{{ $booking->id }}">
                                    <button type="submit"
                                        class="flex justify-center items-center bg-black hover:bg-gray-800 py-2 rounded-lg w-full font-medium text-white text-xs transition-colors">
                                        Terima
                                    </button>
                                </form>
                                <form action="{{ route('booking.reject') }}" method="POST" class="w-full">
                                    ...
                                    <input type="hidden" name="id" value="{{ $booking->id }}">
                                    <button type="submit"
                                        class="flex justify-center items-center hover:bg-red-50 py-2 border border-red-200 hover:border-red-300 rounded-lg w-full font-medium text-red-600 text-xs transition-colors">
                                        Tolak
                                    </button>
                                </form>
                            </div>
            ...
            <div class="flex flex-col gap-4">
                <div class="flex justify-between items-center">
                    ...
                    @if ($cancellationProposals->count() > 0)
                        <span
                            class="bg-orange-500 px-2 py-0.5 rounded-full font-bold text-white text-xs">{{ $cancellationProposals->count() }}</span>
                    @endif
                </div>
                ...
                <div class="flex flex-col gap-3">
                    @forelse($cancellationProposals as $booking)
                        <div
                            class="flex flex-col gap-3 bg-red-50 hover:shadow-md p-4 border border-red-100 rounded-xl transition-all duration-200">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h3 class="font-semibold text-gray-900 text-base">{{ $booking->guest_name }}</h3>
                                    <p class="text-gray-500 text-xs">Ingin membatalkan: {{ $booking->eventType->name }}
                                    </p>
                                </div>
                                <div class="text-right">
                                    <p class="font-medium text-gray-900 text-xs">
                            ...
                            <div class="gap-2 grid grid-cols-2 mt-1">
                                <form action="{{ route('booking.accept') }}" method="POST" class="w-full">
                                    ...
                                    <input type="hidden" name="id" value="{{ $booking->id }}">
                                    <button type="submit"
                                        class="flex justify-center items-center bg-red-600 hover:bg-red-700 py-2 rounded-lg w-full font-medium text-white text-xs transition-colors">
                                        Setujui
                                    </button>
                                </form>
                                <form action="{{ route('booking.reject') }}" method="POST" class="w-full">
                                    ...
                                    <input type="hidden" name="id" value="{{ $booking->id }}">
                                    <button type="submit"
                                        class="flex justify-center items-center hover:bg-white py-2 border border-gray-300 rounded-lg w-full font-medium text-gray-700 text-xs transition-colors">
                                        Tolak
                                    </button>
                                </form>
                            </div>
